...
def main():
    ...
    input_fmri = load_fmri(args.input) ## TODO: Normalize fMRI
    ...
    with torch.no_grad(), autocast("cuda"):
        ...
        input_fmri = 2 * torch.tensor(np.array(input_fmri)) 
        input_fmri = input_fmri.to(next(model.parameters()).device)        
        cond["c_concat"] = [model.fmri2visual_model(input_fmri)]
