...
def filter_optimized_params(model, args):
    if args.filter_mode == 'no_filter':
        ...
        filtered_params = [param for name, param in model.named_parameters() if param.requires_grad is True and 'control_model.adaptor_blocks' in name ]
        filtered_names = [name for name, param in model.named_parameters() if param.requires_grad is True and 'control_model.adaptor_blocks' in name]
