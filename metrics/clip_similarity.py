...
class ClipSimilarity(nn.Module):
    ...
    def forward(
        self, image_0: torch.Tensor, image_1: torch.Tensor, ref_image_0=None, ref_image_1=None, mode='visual_guidance',
            instruction_text=None, captions_text=None
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        image_features_0 = self.encode_image(image_0)
        image_features_1 = self.encode_image(image_1)
        sim_0, sim_1 = torch.Tensor([0, ]), torch.Tensor([0, ])

        if mode == 'visual_guidance':
        ...
        if mode == 'language_guidance':
            # print(instruction_text)
            text_features_delta = self.encode_text(instruction_text)
            sim_direction = F.cosine_similarity(image_features_1 - image_features_0, text_features_delta)

        if mode == 'captions_guidance':
            text_0, text_1 = captions_text
