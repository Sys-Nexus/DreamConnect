...
ind = np.arange(0,10)
# tot_img = 982
tot_img = all_brain_recons.shape[0]
...
effnet = np.array([sp.spatial.distance.correlation(gt[i],fake[i]) for i in range(len(fake))]).mean()
