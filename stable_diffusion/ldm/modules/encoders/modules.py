...
class ExFrozenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14", device="cuda", max_length=77):
        ...
        def embedding_forward(
                ...
                if inputs_embeds is None:
                    # print('CLIP->embedding_forward->inputs_embeds=None')
                ...
                if embedding_manager is not None:
                    ...
                    for i in range(len(inputs_embeds)):
                        embeddings.append(inputs_embeds[i] + position_embeddings)
        ...
        def text_encoder_forward(
            ...
            hidden_states = self.embeddings(input_ids=input_ids, position_ids=position_ids, 
                                            embedding_manager=embedding_manager, prospect_words=prospect_words)
            ...
            # https://github.com/openai/CLIP/blob/cfcffb90e69f37bf2ff1e988237a0fbe41f33c04/clip/model.py#L324
                causal_attention_mask = _build_causal_attention_mask(bsz, seq_len, hidden_state.dtype).to(
                    hidden_state.device
                )
                ...
                if attention_mask is not None:
                    ...
                    attention_mask = _expand_mask(attention_mask, hidden_state.dtype)
...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def forward(self, text, is_return_pool=False):
        batch_encoding = self.tokenizer(text, truncation=True, max_length=self.max_length, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")
        tokens = batch_encoding["input_ids"].to(self.device)
        ...
        if is_return_pool:
            ...
            return z, z[torch.arange(z.shape[0]), :14]#adapt_pool_z / torch.norm(adapt_pool_z, dim=-1, keepdim=True) # z_pooled.unsqueeze(1) / torch.norm(z_pooled.unsqueeze(1), dim=-1, keepdim=True) #adapt_pool_z# z[torch.arange(z.shape[0]), tokens.argmax(dim=-1)-1].unsqueeze(1) #z[torch.arange(z.shape[0]), 0].unsqueeze(1)  # z[torch.arange(z.shape[0]), tokens.argmax(dim=-1)].unsqueeze(1) #torch.mean(z,keepdims=True,dim=1)# pooler_output.unsqueeze(1)
        else:
            return z
