...
class model_module(pl.LightningModule):
    ...
    def inference(self, xtype=[], condition=[], condition_types=[], n_samples=1,
                  mix_weight={'video': 1, 'audio': 1, 'text': 1, 'image': 1, 'fmri': 1},
                  image_size=256, ddim_steps=50, scale=7.5, num_frames=8):
        net = self.net
        ...
        for i, condition_type in enumerate(condition_types):
            ...
            elif condition_type == 'fmri':
                # import pdb; pdb.set_trace()
                # ctemp1 = regularize_fmri(condition[i]).cuda()
                ctemp1 = condition[i].cuda()
                ctemp1 = ctemp1[None].repeat(n_samples, 1, 1)
                cim = net.clip_encode_fmri(ctemp1).cuda()
                uim = None
                # import pdb; pdb.set_trace()
                # scale = 1.1
                if scale != 1.0:
                    dummy = torch.zeros_like(ctemp1).cuda()
                    uim = net.clip_encode_fmri(dummy).cuda()
                conditioning.append(torch.cat([uim, cim]))
